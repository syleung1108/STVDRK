...
function [tout, yout] = tvdrk3_proj(FunFcn, tspan, y0, ssize)
...
while (k < N)
  ...
  y1 = y + h*feval(FunFcn, t, y); y1 = y1/norm(y1);
  y2 = y1 + h*feval(FunFcn, t+h, y1); y2 = y2/norm(y2);
  y3 = 0.75*y+0.25*y2; y3 = y3/norm(y3);
  y4 = y3 + h*feval(FunFcn, t+0.5*h, y3); y4 = y4/norm(y4);
  y = (y+2*y4)/3;
  y = y'/norm(y);
